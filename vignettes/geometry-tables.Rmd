...
- identifiers on parts, ggplot understands this and maps provides it
...
mapstidy <- function(x) {
  ...
  regions <- cumsum(bad)
  coords <- tibble::as_tibble(list(x_ = x$x, y_ = x$y, count = regions))[!bad, ]
  obj <- unlist(lapply(strsplit(x$names, ":"), "[", 1L))
  part <- unlist(lapply(strsplit(x$names, ":"), tail, 1L))
  coords$object_ <- obj[coords$count + 1L]
  coords$branch_ <- part[coords$count + 1L]
  coords$island_ <- TRUE  ## hmm needs work
  coords$order_ <- seq(nrow(coords))
  
  
  spbabel::sp(coords, attr_tab = tibble::tibble(region = obj))         
  }
